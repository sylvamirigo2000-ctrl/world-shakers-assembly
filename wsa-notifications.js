...
class WSANotifications {
    ...
    return `
      <div class="notif-item ${isRead}" data-id="${notification.id}">
        ...
        <div class="notif-actions">
          <button class="btn btn-small btn-primary" onclick="window.wsa_notif.goToSource('${notification.id}')">View</button>
  ...
  /**
   * Navigate to the tab/page a notification originated from.
   * Uses notification.action_url when present (e.g. on member-facing
   * pages that link to a specific record); otherwise falls back to a
   * type -> location map so "View" always lands somewhere useful,
   * even on single-page dashboards like admin.html that use tabs
   * instead of separate URLs.
   */
  goToSource(notificationId) {
    ...
    const typeToTab = {
      ...
      'feedback': 'feedback',
      'request': 'leadership-requests-oversight',
      'message': 'messaging',
      'milestone': 'notifications'
    };
    const tabName = typeToTab[notification.notification_type];

    // Prefer switching tabs in-page over a full navigation — a full page
    // reload just lands back on the Dashboard tab (the default tab on
    // load), which looks like the click "did nothing" or bounced back.
